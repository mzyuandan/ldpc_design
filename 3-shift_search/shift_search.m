...
g4s = 1000*ones(1,6);
g6s = 1000*ones(1,6);
set_g4 = [100 100 100 100 100 0];
set_g6 = [10000 10000 10000 10000 10000 0];
...
%% 赋初值，输出无四环
 ...
 while(1)
     ...
     if (girth_cmp(g4s, set_g4)==0)
        ...
        H_c_b5(1,K+1) = floor(gg*Z5/z_max);
        H_c_b6(1,K+1) = floor(gg*Z6/z_max);
        H_c_b0(M,K+1) = gg;
        H_c_b1(M,K+1) = floor(gg*Z1/z_max);
        H_c_b2(M,K+1) = floor(gg*Z2/z_max);
        H_c_b3(M,K+1) = floor(gg*Z3/z_max);
        H_c_b4(M,K+1) = floor(gg*Z4/z_max);
        H_c_b5(M,K+1) = floor(gg*Z5/z_max);
        H_c_b6(M,K+1) = floor(gg*Z6/z_max);
        
        H_c_b0(xpos,K+1) = fix(z_max*rand);
        H_c_b1(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z1/z_max);
        H_c_b2(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z2/z_max);
        H_c_b3(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z3/z_max);
        H_c_b4(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z4/z_max);
        H_c_b5(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z5/z_max);
        H_c_b6(xpos,K+1) = floor(H_c_b0(xpos,K+1)*Z6/z_max);
            ...
            while(1)
                ...
                for jj=1:2:7
                    if (g4_pos(ii,jj+1)>=K+1)
                        ...
                        H_c_b5(g4_pos(ii,jj),g4_pos(ii,jj+1)) = floor(H_c_b0(g4_pos(ii,jj),g4_pos(ii,jj+1))*Z5/z_max);
                        H_c_b6(g4_pos(ii,jj),g4_pos(ii,jj+1)) = floor(H_c_b0(g4_pos(ii,jj),g4_pos(ii,jj+1))*Z6/z_max);
                    end
                end                      
